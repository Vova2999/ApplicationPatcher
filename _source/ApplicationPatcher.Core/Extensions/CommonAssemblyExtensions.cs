...
namespace ApplicationPatcher.Core.Extensions {
	public static class CommonAssemblyExtensions {
		...
		public static CommonType GetCommonTypeFromThisAssembly(this CommonAssembly commonAssembly, Type type, bool throwExceptionIfNotFound = false) {
			return (commonAssembly.Load().TypeReflectionToType.TryGetValue(type, out var commonTypes) ? commonTypes : Enumerable.Empty<CommonType>()).WhereFrom(commonAssembly).SingleOrDefault(throwExceptionIfNotFound, type.FullName);
		}
		...
		public static CommonType GetCommonTypeFromThisAssembly(this CommonAssembly commonAssembly, string typeFullName, bool throwExceptionIfNotFound = false) {
			return (commonAssembly.Load().TypeFullNameToType.TryGetValue(typeFullName, out var commonTypes) ? commonTypes : Enumerable.Empty<CommonType>()).WhereFrom(commonAssembly).SingleOrDefault(throwExceptionIfNotFound, typeFullName);
		}
